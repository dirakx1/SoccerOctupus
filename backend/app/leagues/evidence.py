...
an EPL prediction only when its response is real, identifiable, and available
...
class SofaScoreLeagueAdapter(_Adapter):
    ...
    def collect(self, home: str, away: str, kickoff: datetime) -> ProviderEvidence:
        ...
        for team in (home, away):
            ...
            if not row:
                return _excluded(self.provider, f"no verified EPL club identity for {team}", f"{self.base_url}/search/all")
...
class FotMobLeagueAdapter(_Adapter):
    ...
    def __init__(self, *, season: str = "2026/2027", **kwargs: Any):
    ...
    def collect(self, home: str, away: str, kickoff: datetime, *, season: str | None = None) -> ProviderEvidence:
        ...
        payload, error = self._get_json(self.league_url, params={"id": 47, "season": season}, headers=self.headers)
        if error:
            return _unavailable(self.provider, error, self.league_url)
        details = payload.get("details", {}) if isinstance(payload, dict) else {}
        if details.get("id") != 47 or details.get("name") != "Premier League" or details.get("selectedSeason") != season:
            return _excluded(self.provider, "league response did not verify Premier League season", self.league_url)
        ...
        for team, team_id in verified_ids.items():
            detail, detail_error = self._get_json(self.team_url, params={"id": team_id, "ccode3": "ENG"}, headers=self.headers)
            if detail_error:
            ...
            if (
                ...
                or team_details.get("primaryLeagueId") != 47
            ):
                return _excluded(self.provider, f"team response did not verify {team} in Premier League", self.team_url)
        ...
        return ProviderEvidence(self.provider, ADMITTED, f"{self.league_url}; {self.team_url}", fetched_at.isoformat(), "verified Premier League table, club identity, and current pre-kickoff stats", normalized_teams)


class Scores365LeagueAdapter(_Adapter):
    provider = "365Scores"
    base_url = "https://webws.365scores.com/web/games/"
    competition_id = 7  # verified live payload: England Premier League
    headers = {
        "Accept": "application/json",
    ...
    def collect(self, home: str, away: str, kickoff: datetime) -> ProviderEvidence:
        ...
        if not candidates:
            return _excluded(self.provider, "no verified pre-kickoff EPL fixture with competitionId=7", self.base_url)
        event_time, game = candidates[0]
        odds = game.get("odds")
        if not isinstance(odds, dict):
            return _excluded(self.provider, "verified EPL fixture has no odds in current 365Scores payload", self.base_url)
        values = []
        for key in ("homeOdds", "drawOdds", "awayOdds", "1", "X", "2"):
            if key in odds:
                try:
                    values.append(float(odds[key]))
                except (TypeError, ValueError):
                    pass
        if len(values) < 3 or any(value <= 1 for value in values[:3]):
            return _excluded(self.provider, "fixture odds are malformed", self.base_url)
        home_odd, draw_odd, away_odd = values[:3]
        raw = [1 / home_odd, 1 / draw_odd, 1 / away_odd]
        total = sum(raw)
        return ProviderEvidence(self.provider, ADMITTED, self.base_url, _now(), "verified pre-kickoff EPL market odds", {"kickoff": event_time.isoformat(), "homeImplied": round(raw[0] / total, 4), "drawImplied": round(raw[1] / total, 4), "awayImplied": round(raw[2] / total, 4), "marketMargin": round(total - 1, 4)})
...
class YouTubeLeagueAdapter(_Adapter):
    ...
    def collect(self, home: str, away: str, kickoff: datetime) -> ProviderEvidence:
        ...
        for team in (home, away):
            ...
            if not videos:
                return _excluded(self.provider, f"no verified pre-kickoff EPL videos for {team}", self.search_url)
        ...
        return ProviderEvidence(
            ...
            self.search_url,
            _now(),
            "verified team-specific pre-kickoff EPL video metadata",
    ...
    def _search_team(self, team: str, kickoff: datetime) -> tuple[list[dict[str, Any]], str | None]:
        ...
        payload, error = self._get_json(
            ...
            params={
                "part": "snippet",
                "q": f"{alias} Premier League tactical analysis form highlights",
                "type": "video",
                "maxResults": 5,
...
class ZepLeagueAdapter(_Adapter):
    ...
    def __init__(self, api_key: str = "", graph_id: str = "", **kwargs: Any):
        super().__init__(**kwargs)
        self.api_key, self.graph_id = api_key.strip(), graph_id.strip()

    def collect(self, home: str, away: str, kickoff: datetime) -> ProviderEvidence:
        if not self.api_key or not self.graph_id:
            reason = "edition-scoped EPL graph ID is not configured" if self.api_key else "Zep API key is not configured"
        ...
        try:
            ...
            results = client.graph.search(query=f"Premier League {home} {away}", graph_id=self.graph_id, limit=5)
...
class OptaLeagueAdapter(_Adapter):
    ...
    def collect(self, home: str, away: str, kickoff: datetime) -> ProviderEvidence:
        ...
        payload, error = self._get_json(f"{self.base_url}/squads", params={"_rt": "b", "_fmt": "json", "_ak": self.api_key, "comp": "eng.1"})
        if error:
        ...
        if not any(_team_matches(value, home) for value in names) or not any(_team_matches(value, away) for value in names):
            return _excluded(self.provider, "configured Opta response did not verify both EPL clubs", f"{self.base_url}/squads")
...
def collect_league_evidence(*, home: str, away: str, kickoff: datetime, settings: RuntimeSettings, season: str = "2026-27", graph_id: str = "", get: Callable[..., Any] = requests.get) -> list[dict[str, Any]]:
    ...
    adapters: Iterable[_Adapter] = (
        SofaScoreLeagueAdapter(get=get),
        FotMobLeagueAdapter(season=f"{season[:4]}/{int(season[:4]) + 1}", get=get),
        Scores365LeagueAdapter(get=get),
        YouTubeLeagueAdapter(api_key=settings.youtube_api_key, get=get),
        ZepLeagueAdapter(api_key=settings.zep_api_key, graph_id=graph_id, get=get),
        OptaLeagueAdapter(api_key=settings.opta_api_key, base_url=settings.opta_base_url, get=get),
    )
...
def specialist_reports(provider_evidence: Iterable[dict[str, Any]], *, fotmob_adjustment_applied: bool = False) -> list[dict[str, Any]]:
    ...
    reports = [
        ...
        {"name": "Recent form", "status": "active", "numericContribution": True, "source": "ESPN completed results", "freshness": "pre-kickoff", "reason": "Five-match points form is included in the baseline."},
    ]
